    ...
    >
      ...
      <header className="relative z-[100] px-4 md:px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className={`flex items-center gap-2 ${isRTL ? 'order-3 flex-row-reverse' : 'order-1'}`}>
            <VivoraLogo
              size={isRTL ? 'sm' : 'md'}
              className={isRTL ? 'ml-1 origin-left scale-95' : ''}
            />
            <span className="bg-white/20 text-white px-2 py-0.5 rounded-full hidden sm:inline text-xs">{isRTL ? 'BETA' : 'BETA'}</span>
          </div>
          ...
          <div className={`flex items-center gap-2 md:gap-3 ${isRTL ? 'order-1 flex-row-reverse' : 'order-3'}`}>
            {user ? (
              <>
                <UserMenuDropdown
                  user={user}
        ...
        >
          <form onSubmit={handleSubmit}>
            ...
            >
              ...
              <div className={`flex items-center justify-between px-3 md:px-6 py-3 md:py-4 border-t border-gray-100 ${isRTL ? 'flex-row' : 'flex-row-reverse'}`}>
                <div className={`flex items-center gap-1.5 md:gap-3 ${isRTL ? 'flex-row' : 'flex-row-reverse'}`}>
                  {isRTL ? (
                    <>
                      <motion.button
                        type="submit"
                      ...
                      >
                        <Send className="w-4 h-4 md:w-5 md:h-5 text-gray-600 rotate-180" />
                      </motion.button>
                      ...
                      <div className="relative">
                        <button
                          ...
                          className="flex items-center gap-1 md:gap-2 px-2 md:px-3 py-1 md:py-1.5 text-gray-500 hover:bg-gray-100 rounded-lg transition-colors text-xs md:text-sm"
                        >
                    ...
                    <>
                      <div className="relative">
                        <button
                          type="button"
                          onClick={() => setShowVisibilityMenu(!showVisibilityMenu)}
                          className="flex items-center gap-1 md:gap-2 px-2 md:px-3 py-1 md:py-1.5 text-gray-500 hover:bg-gray-100 rounded-lg transition-colors text-xs md:text-sm flex-row-reverse"
                        >
                          <ChevronDown className="w-3 h-3" />
                          <span className="hidden sm:inline">{isPublic ? t('home.public') : t('home.private')}</span>
                          {isPublic ? <Globe className="w-3.5 h-3.5 md:w-4 md:h-4" /> : <Lock className="w-3.5 h-3.5 md:w-4 md:h-4" />}
