...
export const HomePage: React.FC<HomePageProps> = ({
  ...
  const COLOR_THEMES = [
    ...
    { name: 'Pulse', colors: ['#EC4899', '#F472B6', '#FBCFE8'], desc: 'Vivora Pink' },
    ...
    { name: 'Crimson', colors: ['#E11D48', '#FB7185', '#FFE4E6'], desc: 'Crimson Rose' },
  ...
  const uploadImmediately = useCallback((files: File[]) => {
    ...
    if (!image) {
      toast({ title: 'Images only', description: 'Vivora X accepts one PNG, JPEG, WebP, or GIF image only.', variant: 'destructive' });
      return;
    }
    ...
    const newEntries = [image].map(file => ({
      file,
    ...
    for (const entry of newEntries) {
      readImageAsDataUrl(entry.file).then(url => {
        if (!url) {
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    uploadImmediately(Array.from(e.dataTransfer.files));
  };
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    uploadImmediately(Array.from(e.target.files || []));
    e.target.value = '';
  };
                      ...
                      <>
                        ...
                        <motion.div
                          ...
                          style={{
                            ...
                            top: plusButtonRef.current ? plusButtonRef.current.getBoundingClientRect().top - 145 : 'auto',
                          }}
                        >
                          {/* Attach Images */}
                          <button
