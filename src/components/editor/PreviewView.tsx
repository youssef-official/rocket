...
export default defineConfig({
  ...
  server: {
    ...
    if (projectId && spFiles['/index.html']) {
      const analyticsSrc = `<script>window.__vivorax_project_id="${projectId}";window.__vivorax_api_base="${SUPABASE_URL}";(function(){if(window.__vxa)return;window.__vxa=true;var SK="vx_s",FK="vx_fq",PID=window.__vivorax_project_id,API=window.__vivorax_api_base;function uid(){return"xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,function(c){var r=Math.random()*16|0;return(c==="x"?r:(r&3)|8).toString(16)})}function gs(){var s=sessionStorage.getItem(SK);if(s)return JSON.parse(s);var o={id:uid(),start:Date.now(),pages:[],device:/Mobi|Android/i.test(navigator.userAgent)?"mobile":"desktop",country:Intl.DateTimeFormat().resolvedOptions().timeZone||"unknown",referrer:document.referrer||"direct",sw:screen.width,sh:screen.height};sessionStorage.setItem(SK,JSON.stringify(o));return o}function ss(o){sessionStorage.setItem(SK,JSON.stringify(o))}function qe(e){try{var q=JSON.parse(localStorage.getItem(FK)||"[]");q.push(e);if(q.length>200)q=q.slice(-200);localStorage.setItem(FK,JSON.stringify(q))}catch(x){}}function flush(){if(!PID||!API)return;var r=localStorage.getItem(FK);if(!r)return;var ev;try{ev=JSON.parse(r)}catch(x){return}if(!ev||!ev.length)return;localStorage.removeItem(FK);var url=API+"/functions/v1/track-analytics";var p=JSON.stringify({project_id:PID,events:ev});if(navigator.sendBeacon)navigator.sendBeacon(url,new Blob([p],{type:"application/json"}));else fetch(url,{method:"POST",headers:{"Content-Type":"application/json"},body:p,keepalive:true}).catch(function(){})}function tv(){var s=gs();s.pages.push({path:location.pathname,time:Date.now()});ss(s);qe({session_id:s.id,event_type:"pageview",path:location.pathname,device:s.device,referrer:s.referrer,country:s.country,screen_w:s.sw,screen_h:s.sh})}function fin(){var s=gs();qe({session_id:s.id,event_type:"session_end",path:location.pathname,device:s.device,referrer:s.referrer,country:s.country,screen_w:s.sw,screen_h:s.sh,duration:Math.round((Date.now()-s.start)/1000),pages_count:s.pages.length});flush()}tv();var lp=location.pathname;setInterval(function(){if(location.pathname!==lp){lp=location.pathname;tv()}},500);setInterval(flush,30000);addEventListener("beforeunload",fin)})();</script>`;
