...
import { Music, Play, Pause, SkipForward, SkipBack, Volume2, VolumeX, Repeat, X, Plus, Heart, Trash2, ChevronDown, GripVertical, HardDrive } from 'lucide-react';
...
const PLAYLIST_KEY = 'vivora_music_playlist';
...
type LocalPlaylist = { tracks: Track[]; currentIndex: number; volume: number; repeatMode: boolean };

const loadPlaylist = (): LocalPlaylist => {
  try {
    const parsed = JSON.parse(localStorage.getItem(PLAYLIST_KEY) || '{}') as Partial<LocalPlaylist>;
    const tracks = Array.isArray(parsed.tracks)
      ? parsed.tracks.filter(track => track && typeof track.id === 'string' && typeof track.title === 'string' && typeof track.url === 'string')
      : [];
    return {
      tracks,
      currentIndex: Math.max(0, Math.min(Number(parsed.currentIndex) || 0, Math.max(tracks.length - 1, 0))),
      volume: Math.max(0, Math.min(Number(parsed.volume ?? 0.7), 1)),
      repeatMode: Boolean(parsed.repeatMode),
    };
  } catch {
    return { tracks: [], currentIndex: 0, volume: 0.7, repeatMode: false };
  }
};

function useLocalPlaylist() {
  const initial = useRef<LocalPlaylist | null>(null);
  if (!initial.current) initial.current = loadPlaylist();
  const [tracks, setTracks] = useState<Track[]>(initial.current.tracks);
  const [currentIndex, setCurrentIndex] = useState(initial.current.currentIndex);
  const [volume, setVolume] = useState(initial.current.volume);
  const [repeatMode, setRepeatMode] = useState(initial.current.repeatMode);

  useEffect(() => {
    localStorage.setItem(PLAYLIST_KEY, JSON.stringify({ tracks, currentIndex, volume, repeatMode }));
  }, [tracks, currentIndex, volume, repeatMode]);

  useEffect(() => {
    if (currentIndex >= tracks.length) setCurrentIndex(Math.max(tracks.length - 1, 0));
  }, [tracks.length, currentIndex]);

  return { tracks, setTracks, currentIndex, setCurrentIndex, volume, setVolume, repeatMode, setRepeatMode };
}
        ...
        {isHidden && (
          ...
          >
            <button onClick={() => setIsHidden(false)}
              className="w-11 h-11 rounded-full bg-gradient-to-br from-pink-600 to-fuchsia-600 shadow-lg shadow-pink-500/40 flex items-center justify-center hover:shadow-xl transition-all relative">
            ...
            {!expanded ? (
              ...
              >
                ...
                <button onClick={() => setExpanded(true)}
                  className="w-9 h-9 rounded-full bg-gradient-to-br from-pink-500/30 to-fuchsia-500/30 border border-white/10 flex items-center justify-center relative flex-shrink-0">
              ...
              >
                ...
                <div className="flex items-center justify-between px-4 py-2.5 cursor-grab active:cursor-grabbing"
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <HardDrive className="w-3 h-3 text-pink-300/70" aria-label="Saved on this device" />
                  </div>
        ...
        {(['player', 'playlist'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`flex-1 py-2 text-[11px] font-semibold uppercase tracking-wider transition-colors ${
              tab === t ? 'text-pink-400 border-b-2 border-pink-400' : 'text-white/30 hover:text-white/50'
            }`}>
        ...
        <div className="p-5">
          ...
          <div className="flex items-center justify-center mb-5">
            <motion.div
              ...
              className="w-24 h-24 rounded-full bg-gradient-to-br from-pink-600/20 via-fuchsia-600/20 to-purple-600/20 border border-white/10 flex items-center justify-center relative"
            >
          ...
          <div className="mb-5">
            <div data-no-drag className="w-full h-2 bg-white/[0.06] rounded-full cursor-pointer group relative" onClick={handleSeek}>
              <div className="h-full bg-gradient-to-r from-pink-500 to-fuchsia-500 rounded-full transition-all relative"
                style={{ width: `${progress}%` }}>
                <div className="absolute right-0 top-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-white shadow-lg shadow-pink-500/30 opacity-0 group-hover:opacity-100 transition-opacity" />
              </div>
            </div>
          ...
          <div className="flex items-center justify-center gap-4 mb-5">
            <button onClick={() => setRepeatMode(!repeatMode)}
              className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${repeatMode ? 'bg-pink-500/20 text-pink-400' : 'text-white/25 hover:text-white/50'}`}>
            ...
            <button onClick={togglePlay} disabled={tracks.length === 0}
              className="w-14 h-14 rounded-full bg-gradient-to-br from-pink-500 to-fuchsia-600 hover:from-pink-400 hover:to-fuchsia-500 flex items-center justify-center text-white shadow-xl shadow-pink-500/30 transition-all disabled:opacity-30 active:scale-95">
          ...
          <div data-no-drag className="flex items-center gap-3 px-2">
            ...
            <input type="range" min="0" max="1" step="0.02" value={volume}
              ...
              className="flex-1 h-1 bg-white/[0.06] rounded-full appearance-none [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-pink-400 [&::-webkit-slider-thumb]:cursor-pointer" />
              ...
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden mb-2">
                <div className="space-y-2 p-3 bg-white/[0.03] rounded-xl border border-white/[0.06]">
                  <input data-no-drag value={newTitle} onChange={e => setNewTitle(e.target.value)} placeholder="Track title..."
                    className="w-full px-3 py-2 bg-white/[0.04] border border-white/[0.08] rounded-lg text-xs text-white placeholder:text-white/20 outline-none focus:border-pink-500/40" />
                  <input data-no-drag value={newUrl} onChange={e => setNewUrl(e.target.value)} placeholder="MP3 URL..."
                    className="w-full px-3 py-2 bg-white/[0.04] border border-white/[0.08] rounded-lg text-xs text-white placeholder:text-white/20 outline-none focus:border-pink-500/40 font-mono" />
                  <button onClick={handleAdd} disabled={!newUrl.trim()}
                    className="w-full py-2 bg-pink-500/20 hover:bg-pink-500/30 text-pink-300 text-xs font-medium rounded-lg transition-colors disabled:opacity-30">
                    Add to Playlist
                  </button>
                </div>
              </motion.div>
            ...
            {tracks.map((track, idx) => (
              <div key={track.id}
                className={`flex items-center gap-2 px-3 py-2.5 rounded-xl transition-all cursor-pointer ${
                  idx === currentIndex ? 'bg-pink-500/10 border border-pink-500/20' : 'hover:bg-white/[0.03] border border-transparent'
                }`}
                ...
                <div className="w-7 h-7 rounded-lg bg-white/[0.04] flex items-center justify-center flex-shrink-0">
                  {idx === currentIndex && isPlaying ? (
                    <div className="flex items-end gap-[2px] h-3">
                      {[1, 2, 3].map(i => (
                        <motion.div key={i} className="w-[2px] bg-pink-400 rounded-full"
                ...
                {favorites.map(track => {
                  ...
                  return (
                    <div key={track.id} onClick={() => playTrack(idx)}
                      className={`flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-white/[0.03] transition-colors cursor-pointer ${idx === currentIndex ? 'bg-pink-500/10' : ''}`}>
